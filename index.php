...
if (isset($_POST['submit'])) {
    ...
    if ($country_code !== '') {
        ...
        if (strpos($country_code, '+') !== 0) $country_code = '+' . $country_code;
    }
    ...
    if (!$con) {
        die("Connection failed: Unable to connect to PostgreSQL server.");
    }

    if (empty($error)) {
        $query = "INSERT INTO leads (name, email, location, mobile, inquiry, source, message, consent, newsletter) 
                  VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)";
        $result = pg_query_params($con, $query, [
            $name, $email, $location, $mobile, $inquiry, $source, $message, $consent, $newsletter
        ]);
        ...
        <form action="" method="post" enctype="multipart/form-data" novalidate>
            <div class="form-grid">
                <div class="field">
                    ...
                    <input id="name" type="text" name="name" placeholder="Enter full name" required value="<?php echo htmlspecialchars($_POST['name'] ?? ''); ?>">
                </div>
                <div class="field">
                    <label for="email">Email <span class="req">*</span></label>
                    <input id="email" type="email" name="email" placeholder="Enter email" required value="<?php echo htmlspecialchars($_POST['email'] ?? ''); ?>">
                </div>
                <div class="field">
                ...
                <div class="field">
                    ...
                    <input id="mobile" type="text" name="mobile" placeholder="+X XX XXXXXXXX" required value="<?php echo htmlspecialchars($_POST['mobile'] ?? ''); ?>">
                </div>
            </div>
            <div class="field">
